...
def main(_):
    ...
    for inputs_dict in batch(eval_features, batch_size):
        ...
        if not FLAGS.trt_engine:
            label_ids_data = np.stack(inputs_dict[label_id_key]).astype('int32')
        input_ids_data = np.stack(inputs_dict['input_ids']).astype('int32')
        input_mask_data = np.stack(inputs_dict['input_mask']).astype('int32')
        segment_ids_data = np.stack(inputs_dict['segment_ids']).astype('int32')
        ...
        inputs.append(tritongrpcclient.InferInput('input_mask', input_mask.shape, "INT32"))
