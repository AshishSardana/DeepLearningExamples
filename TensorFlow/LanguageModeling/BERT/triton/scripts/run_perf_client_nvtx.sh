...
OUTPUT=${OUTPUT:-"triton_tf_fp32_bs1"}
...
then
    if [ ! "$(docker inspect -f "{{.State.Running}}" triton_server_cont)" = "true" ] ; then
        ...
        echo "Launching TRITON server and saving the profile as $(echo "$OUTPUT") and running for $(echo "$DURATION")"
        bash triton/scripts/launch_server_nvtx.sh $OUTPUT $DURATION
...
bash triton/scripts/launch.sh mkdir -p /results/perf_client/${MODEL_NAME}
