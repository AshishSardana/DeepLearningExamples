...
def extract_run_squad_flags():
  ...
  flags.DEFINE_bool("use_tftrt", False, "Whether to use TF-TRT")
...
def get_frozen_tftrt_model(bert_config, shape, use_one_hot_embeddings, init_checkpoint):
  print("*********INSIDE FROZEN TFTRT MODEL***********")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if not is_training and FLAGS.use_trt:
        ...
        trt_graph = get_frozen_tftrt_model(bert_config, features, use_one_hot_embeddings, init_checkpoint)
...
def export_model(estimator, export_dir, init_checkpoint):
    ...
    if FLAGS.use_tftrt == True:
        print("Replacing the saved_model.pb with frozen_modelTRT.pb in the above model dir since TF-TRT is activated.")
        shutil.copy("frozen_modelTRT.pb", final_model_folder+"/saved_model.pb")
        shutil.rmtree(final_model_folder+"/variables/")
