...
def get_features_keys():
    features_keys = list(HASH_BUCKET_SIZES.keys()) + \
                    FLOAT_COLUMNS_SIMPLE_BIN_TRANSFORM + \
                    list(IDENTITY_NUM_BUCKETS.keys()) + \
                    [column_name + "_log_01scaled" for column_name in FLOAT_COLUMNS_LOG_BIN_TRANSFORM + INT_COLUMNS] + \
                    [DISPLAY_ID_COLUMN, IS_LEAK_COLUMN]
    return features_keys


def get_feature_columns(training_columns=NV_TRAINING_COLUMNS):
    ...
    for column_name in training_columns:
        if column_name in HASH_BUCKET_SIZES:
            ...
            if column_name in FLOAT_COLUMNS_SIMPLE_BIN_TRANSFORM:
                ...
                new_column_name = column_name + "_log_01scaled"
                columns.append(tf.feature_column.numeric_column(
                    new_column_name, shape=(1,)))
            elif column_name in INT_COLUMNS:
                # add a categorical_column for column_name + "_log_int"
                new_column_name = column_name + "_log_01scaled"
                columns.append(tf.feature_column.numeric_column(
                    new_column_name, shape=(1,)))
            wide_columns.extend(columns)
            deep_columns.extend(columns)
            continue
